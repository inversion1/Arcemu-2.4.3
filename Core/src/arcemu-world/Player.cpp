...
void Player::GiveXP(uint32 xp, const uint64 &guid, bool allowbonus)
{
	...
	*/
	...
	if(levelup)
	{
		...
		for (std::set<Object*>::iterator iter = GetInRangeSetBegin(); iter != GetInRangeSetEnd(); iter++)
		{ // Dword: send a quest update for all surrounding NPCs
			uint32 qst_giver_status = sQuestMgr.CalcStatus(*iter, this);
			WorldPacket packet(SMSG_QUESTGIVER_STATUS, 0x0C);
			packet << (*iter)->GetGUID() << qst_giver_status;
...
void Player::OnPushToWorld()
{
	...
	*data << uint32(0);
	...
	if ( m_playerInfo->lastOnline + 900 < UNIXTIME ) // did we log out for more than 15 minutes?
...
void Player::DeathDurabilityLoss(double percent)
{
	...
	Item * pItem;
	if (percent == .1)
	{// Dword: can't send this on spirit healer ressurection
	...
	if (percent == .25)
	{ // Dword: spirit healer ressurection durability loss; also remove durability on unequipped items
		for (int8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
		{
			Container* curBag = static_cast<Container*>(GetItemInterface()->GetInventoryItem(i));
			if (curBag != 0)
			{
				for (int8 j = 0; j < curBag->GetProto()->ContainerSlots; j++)
				{
					pItem = curBag->GetItem(j);
					if (pItem != 0)
					{
						pMaxDurability = pItem->GetUInt32Value(ITEM_FIELD_MAXDURABILITY);
						pDurability =  pItem->GetUInt32Value(ITEM_FIELD_DURABILITY);
		...
		for (int8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; i++)
		{
			...
			if(pItem != 0)
			{
				...
				if(pDurability)
				{
					...
					pItem->SetUInt32Value(ITEM_FIELD_DURABILITY,(uint32)pNewDurability);
					//pItem->m_isDirty = true; this causes a crash
				}
			}
		}
	}
...
// Dword
void Player::RemoveItemVisual(uint8 slot)
{
	uint32 visualIndex = PLAYER_VISIBLE_ITEM_1_0 + (16 * slot);
	SetUInt32Value(visualIndex, 0);
	//for (uint8 i = 1; i < 9; i++)
		//SetUInt32Value(visualIndex + i, 0);
}

// Dword
void Player::RestoreItemVisual(uint8 slot, Item* item)
{
	uint32 visualIndex = PLAYER_VISIBLE_ITEM_1_0 + (16 * slot);
	SetUInt32Value(visualIndex, item->GetEntry());
	for (uint8 i = 0; i < 7; i++)
		SetUInt32Value(visualIndex + i + 1, item->GetUInt32Value(ITEM_FIELD_ENCHANTMENT + 3 * i));
	SetUInt32Value(visualIndex + 8, item->GetUInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID));
}

// Dword
void Player::EventSwapItems(int8 srcslot, int8 dstslot)
{
	GetItemInterface()->SwapItemSlots(srcslot, dstslot);
}